...
class RestroomParser
  ...
  def self.rename_keys #changing headers(keys) from our CSV file to match our PublicParks table columns 
    formmated_data = self.parse.map do |hash|
